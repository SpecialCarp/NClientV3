import Tag from '@/models/Tag';
import TagType from "@/enum/TagType";

// enum Language {
// 	ENGLISH, CHINESE, JAPANESE, UNKNOWN, ALL
// }

const languageImg = {
	chinese: "data:image/gif;base64,",
	japanese: "data:image/gif;base64,",
	english: "data:image/gif;base64,",

}

export default class Language {
	public static CHINESE : Language = new Language("chinese", new Tag(29963, "chinese", null, TagType.LANGUAGE), languageImg["chinese"])
	public static ENGLISH : Language = new Language("english", new Tag(12227, "english", null, TagType.LANGUAGE), languageImg["english"])
	public static JAPANESE : Language = new Language("japanese", new Tag(6346, "japanese", null, TagType.LANGUAGE), languageImg["japanese"])
	public static values : Language[] = [Language.CHINESE, Language.ENGLISH, Language.JAPANESE]

	private name : String;
	private tag : Tag;
	private img : String;

	constructor(name : String, tag : Tag, img : String) {
		// console.log(name, tag, img);
		this.name = name;
		this.tag = tag;
		this.img = img;
	}

	public getName() : String { return this.name; }
	public getTag() : Tag { return this.tag; }
	public getImag() : String { return this.img; }

	public getById(id : Number) : Language {
		return Language.values.find(item => item.getTag().getId() == id)
	}

	public getByIds(ids : Array<Number>) : Language {
		return Language.values.find(item => ids.includes(item.getTag().getId()))
	}

}